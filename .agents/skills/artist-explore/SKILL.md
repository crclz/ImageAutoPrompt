...
本skill适用于 anima 与 noobai，方法论两模型通用；anima 的探索流程另有专属路线（见 workflow 一节）。画师tag的写法因模型而异，见下方「模型差异对照」。画师两模型共用。
...
中心思想：从无artist或者指定的artist开始，途中根据用户的反馈，调整画师组合，最终得到最优画师组合。分两条路线：路线二从简单到复杂（先尝试少量画师无权重组合，逐步复杂化）；路线一（仅anima，强烈建议）为一轮大batch一次性广覆盖。
...
- 若目标模型为 anima：让用户在路线一/路线二中二选一（见「流程」）。强烈建议路线一
- dropout比例 (默认推荐0.5). dropout功能的含义是在每一episode开始时，dropout掉一部分artist，以避免与之前的episode选到过于相似的artist。
- prompt数量：路线二默认下表（需询问用户显式确认是否需要更多）；路线一默认32个，必须强烈警告并让用户显式确认（见路线一）
...
anima 在路线一/路线二中二选一（前置澄清时已确认）；noobai 只走路线二。

#### 路线一：单轮大batch（仅anima，强烈建议）
背景：anima 对画师的理解很差，1画师不如2画师，单画师图无法反映画师的真实水平。路线二漏斗的前几个阶段（单画师等）对 anima 是浪费甚至误导，因此直接跳过，用一轮大batch的2画师组合做广覆盖探索。

⚠️ 强烈警告（必须让用户显式确认后才能开始）：本路线只有1轮，prompt 数量默认32，意味着一次性出32张图、出图耗时很长。

| 持续timestep个数 | 思路                                                          | prompt数量 per timestep | 备注                                                    |
| ------------ | ----------------------------------------------------------- | --------------------- | ----------------------------------------------------- |
| -            | 获取artist. 运行 get_artists.py。后续的artist只能从这里面选择 | -                    |  - |
| 1            | 2画师无权重混合。基于上一步的输出选择你认为适合的画师，组合之间不能重复 | 32（默认，须用户显式确认） | artist_only |
| 0            | 得出结论，将该timestep的前3个画师串告诉用户                               | \-                    | 如果用户想要进行其他探索，那你就轮换使用前3个画师串。                           |

#### 路线二：多轮漏斗（anima 的备选；noobai 使用此路线）
